...
The histogram of all $`\lambda_{is}`$ values gives a system-wide view of
how often establishment conditions are favourable vs. exclusionary.
...
Ranking by mean $`\lambda_{is}`$ highlights consistently permissive
...
We relate invader traits to mean $`\lambda_{is}`$ to identify functional
profiles linked to high or low establishment.
