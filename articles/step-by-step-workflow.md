...
and convex hull membership, and computes resident crowding $`C_{js}`$
...
4.  Builds $`C_{js}`$ from Hellinger weights and **Gaussian trait
    kernels**;
...
to a Gaussian kernel $`K`$ with bandwidth $`\sigma_\alpha`$ (median
positive distance by default) ➔ and computes
$`C_{js} = (W K^\top)_{sj}`$ ➔ with optional row-z standardisation
yields $`C^{(z)}_{js}`$.

:information_source: **Why this matters**: $`C_{js}`$ is the
...
$`\sigma_\alpha`$; prefer robust row-scales when z-scoring.

:zap: **Run
[`prepare_trait_space()`](https://b-cubed-eu.github.io/invasimapr/reference/prepare_trait_space.md)**
to construct the trait space, diagnostics, and $`C_{js}`$ in one step,
adding them to `fit`.
...
centres and scales each site row for $`r_{js}`$ (and for $`C_{js}`$ if
...
:bulb: **Summary.** You now have $`r^{(z)}_{js}`$, $`C^{(z)}_{js}`$, and
$`S^{(z)}_{js}`$ with resident moments saved for invader scaling.
...
random-slope parts for `C_z` and `r_z` ➔ clamps $`\alpha_{is} \ge 0`$ ➔
...
:bar_chart: **Heatmaps** of $`\alpha_{is}`$ and $`\Gamma_{is}`$ that
...
> sensitivity to crowding ($`\alpha_{is}`$) varies across sites and
...
> $`\alpha_{is}`$ values are consistently positive but display subtle
...
> invaders: higher $`\alpha_{is}`$ (yellow-green) means stronger
...
> sensitivity to abiotic scaling ($`\Gamma_{is}`$, second) varies across
> sites and invaders.Each column corresponds to an invader, each row to
> a site, and the colour scale represents the parameter value. Here,
> $`\Gamma_{is}`$ values describe how abiotic suitability scales for
> each invader–site combination. Most values fall near zero to moderate
> positive ranges, but some sites show locally elevated $`\Gamma_{is}`$,
> highlighting environments where abiotic alignment disproportionately
> boosts invasion fitness.
...
signed), $`\theta_i`$ and $`\gamma_i`$, plus $`\alpha_{is}`$,
$`\Gamma_{is}`$ where random slopes support site variation.
...
:bulb: **Summary.** You now have $`\lambda_{is}`$ and $`P_{is}`$ under
