...
$`S^{(z)}`$ - into an invasion fitness surface $`\lambda_{is}`$ and then
into establishment probabilities $`P_{is}`$. Each option is motivated,
precisely defined, and illustrated with minimal code that you can paste
into your analysis.
...
where $`\Gamma_{is}`$ scales the abiotic signal, $`\alpha_{is}`$
...
**Output:** site × invader matrix $`\lambda_{is}`$ plus an optional tidy
long table for mapping and summaries.
...
- $`\theta_0,\ \theta_i,\ \alpha_i,\ \beta_i,\ \beta_i^{(\text{signed})}`$:
  from
  [`derive_sensitivities()`](https://b-cubed-eu.github.io/invasimapr/reference/derive_sensitivities.md).
- $`\alpha_{is}`$: from `site_varying_alpha()` (random slopes on
  $`C_z`$).
- $`\Gamma_{is}`$: either broadcast $`\theta_0`$/$`\theta_i`$ or a
...
:bar_chart: Aggregates $`\lambda_{is}`$ to site means then bins them
...
abiotic gains ($`\Gamma_{is}`$), others intensify crowding penalties
($`\alpha_{is}`$).
...
:bar_chart: Summarises $`\lambda_{is}`$ by site and renders quintile
bands to reveal local departures driven by site-specific abiotic
amplification $`\Gamma_{is}`$ and crowding penalties $`\alpha_{is}`$.
...
> site-varying $`\Gamma_{is}`$ and $`\alpha_{is}`$. Spatial structure
> highlights locations with amplified abiotic gains (high
> $`\Gamma_{is}`$) or stronger crowding (large $`\alpha_{is}`$),
> indicating where establishment pressure is locally enhanced or
> suppressed

:bulb: **Interpretation**: Heatmaps of $`\Gamma_{is}`$ and
$`\alpha_{is}`$ reveal where environment “bites” harder and where
...
[`compute_establishment_probability()`](https://b-cubed-eu.github.io/invasimapr/reference/compute_establishment_probability.md)
maps $`\lambda_{is}`$ to a probability $`P_{is}`$ using a **link
...
:hourglass_flowing_sand: **Inputs and outputs** (at a glance):
**Inputs:** either `lambda_is` **or** the components
$`r^{(z)}, C^{(z)}, S^{(z)}`$ with $`\gamma/\Gamma, \alpha, \beta`$; a
scale (`sigma` or `tau`) depending on method; optional `site_df` for
maps. **Output:** site × invader matrix $`P_{is}`$, a tidy long table,
and ready-to-use maps (site mean, invader ranking, heatmap).
...
$`\mathbb{E}[P_{is}]`$ per site, the expected number of establishing
invaders $`\sum_i P_{is}`$, and optional quintile bands for
...
$`P_{is}=\mathbb{1}\{\lambda_{is} > 0\}`$. Produces maps of the count
...
If $`\lambda_{is}`$ is already computed, you can still produce
